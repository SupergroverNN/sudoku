...
function chechValue(board, row, col, value) {
  return checkSector(board, row, col, value) && checkVerticalHorizontal(board, row, col, value)
}
...
function solve(data) {
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 9; j++) {
      if (data[i][j] === 0) {
        for (let value = 1; value <= 9; value++) { // перебираем все значения от 1 до 9 для данной ячейки
          if (chechValue(data, i, j, value)) { // если все норм то записываем данное значение
            data[i][j] = value;
            if (solve(data)) {
              return true;
            } else {
